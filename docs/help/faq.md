...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Cortana setup](/start/cortana) and [Agent send](/tools/agent-send).
